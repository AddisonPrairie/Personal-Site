async function compile(canvas) {
    ...
    const position = [0., 0.0001, 1.2]; const lookAt = [0., 0., 0.2];
    ...
    fn ptStep(coord : vec2i, oin : ptr<function, vec4f>, din : ptr<function, vec4f>, bin : ptr<function, vec4f>, tin : ptr<function, vec4f>) {
        ...
        if (res.dist >= 0.) {
            ...
            o = o + d * res.dist;
            if (res.imat == 0) {
                c = ggxSmith(&seed, &wi, wo, vec3f(1.), .2);
            }
            if (res.imat == 1) {
                var imat : i32 = checker(o);
                if (imat == 0) {
                    c = lambertDiffuse(&seed, &wi, wo, vec3f(.9));
                } else {
                    c = perfectMirror(&wi, wo, vec3f(.9));
                }
            }

            o += res.norm * 1.001 * eps;
    ...
    fn checker(p : vec3f) -> i32 {
        var ipos : vec3i = vec3i(floor((p + vec3f(10.)) * 10.));
        if ((ipos.y + ipos.x) % 2 == 1) {
            return 1;
        } else {
            return 0;
        }
    ...
    fn fract1(p0 : vec3f) -> f32 {
        var i : f32; var g : f32; var e : f32; var s : f32;
        var p : vec3f = p0;
        s = 5.;
        for (var j = 0; j<6; j++) {
            p = sign(p) * (1.7-abs(p-vec3f(1.7)));
            e=8./clamp(dot(p, p), .3, 5.);
            p=p*(e) + p0 - vec3f(.8, 12., .8);
            s *= e;
        }
    ...
    fn hsvTorgb(hsv : vec3f) -> vec3f {
        return hsv.z * mix(
            ...
            clamp(
                abs(
                    fract(vec3f(hsv.x) + vec3f(1., 2. / 3., 1. / 3.)) * 6. - vec3f(3.)
                ) - 1., 
                vec3f(0.), 
                vec3f(1.)
            ), 
    ...
    fn OB0(z : vec3f) -> f32 {
        return max(fract1(z), length(z) - 1.);
    }
    fn OB1(z : vec3f) -> f32 {
        return z.z;
    }
