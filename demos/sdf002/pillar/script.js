async function compile(canvas) {
    ...
    const position = [1, 1, .6]; const lookAt = [0., 0., .6];
    ...
    fn ptStep(coord : vec2i, oin : ptr<function, vec4f>, din : ptr<function, vec4f>, bin : ptr<function, vec4f>, tin : ptr<function, vec4f>) {
        ...
        if (res.dist >= 0.) {
            ...
            if (res.imat == 0) {
                c = lambertDiffuse(&seed, &wi, wo, vec3f(.8));
            }
            if (res.imat == 1) {
                c = ggxSmith(&seed, &wi, wo, vec3f(0.8), .01);
            }

            b *= c;
            o = o + d * res.dist + res.norm * 2. * eps;
    ...
    fn trace(o : vec3f, d : vec3f) -> RayHit {
        ...
        var bHit : bool = false;
        for (; i < iter; i++) {
            var dist : f32 = DE(o + returned.dist * d);
            if (abs(dist) < eps) {bHit = true; break;}
    ...
    fn fract1(p0 : vec3f) -> f32 {
        var p : vec3f = vec3f(p0.x, p0.z - .5, p0.y + .5) * 5.;
        p.z -= 2.5;
        var s : f32 = 3.;
        var e : f32 = 0.;
        for (var j : i32 = 0; j < 9; j++) {
            e = 3.8 / clamp(dot(p, p), 0., 2.);
            s *= e;
            p = abs(p) * e - vec3f(1., 15., 1.);
        }
        return .2 * length(cross(p, vec3f(1., 1., -1.)*.577))/s;
    }
    ...
    fn OB0(z : vec3f) -> f32 {
        var r2 = rotate2d(z.xy, z.z - .6 + Pi * .25);
        return max(fract1(vec3f(r2, z.z)), length(z) - 3.);
    }
    fn OB1(z : vec3f) -> f32 {
        return min(z.y + .5, z.x + .5);
    }
